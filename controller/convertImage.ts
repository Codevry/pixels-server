 ...
 */
export default async function convertImage(inputBuffer: Buffer, operations: Record<string, string>): Promise<Buffer> {
    ...
    for (const [methodName, argsString] of Object.entries(filteredOperations)) {
        ...
        if (typeof sharpInstance[methodName as keyof SharpManager] === 'function') {
            try {
                let parsedArgs: any[] = [];
                ...
                if (argsString) {
                    ...
                    } else if (methodName === 'rotate' || methodName === 'blur') {
                ...
                sharpInstance = (sharpInstance[methodName as keyof SharpManager] as any)(...parsedArgs);

            } catch (e: any) {
                throw new ErrorObject(400, `Invalid arguments for ${methodName}: ${argsString}. Error: ${e.message || e}`);
            }
        } else {
            throw new ErrorObject(400, `Unsupported image operation: ${methodName}`);
        }
    }
